...
> NOTE (AGLG packaging): This is the "copychief" capability referenced in the AGLG CEO brief (original skill name: `style-learner`, by athola). Packaged from source content supplied by Daniel. The `@modules/*` reference files and the `scribe:shared` / `scribe:slop-detector` dependencies are NOT bundled in this package — they must be present in the runtime for module loads and slop-detector integration to work. The SKILL.md itself is complete and loadable.
