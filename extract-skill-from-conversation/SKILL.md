...
> NOTE (AGLG packaging): This is the "conversation-distillate" capability referenced in the AGLG CEO brief. Packaged from source content supplied by Daniel. The `scripts/`, `references/`, and `examples/` files referenced below, and the external `fabric` CLI dependency, are NOT bundled in this package — they must be provided in the runtime for the full pipeline to execute. The SKILL.md itself is complete and loadable.
